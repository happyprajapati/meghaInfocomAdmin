...
const TableOne = ({ name }) => {
  ...
  useEffect(() => {
    ...
    newUrl = `${import.meta.env.VITE_BASE_URL}/api/products/featured`
      ...
      newUrl = `${import.meta.env.VITE_BASE_URL}/api/admin/allusers?page=0&size=10`
    }

    console.log(name)
    setUrl(newUrl)
    
  }, [name, table.table, value.search])
  ...
  useEffect(()=>{
    ...
    fetch(`${url}`, {
      ...
      .then((res) => {
        if (res.success) {
          if(name != 'images') setData(res.data.content)
...
{name != 'images' && <div className="flex items-center justify-center gap-4 mt-3">
  <button disabled
  ...
  <div className="flex items-center gap-2">
    <button
      className="relative h-10 max-h-[40px] w-10 max-w-[40px] select-none rounded-lg bg-black text-center align-middle font-sans text-xs font-medium uppercase text-white shadow-md shadow-gray-900/10 transition-all hover:shadow-lg hover:shadow-gray-900/20 focus:opacity-[0.85] focus:shadow-none active:opacity-[0.85] active:shadow-none disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none"
      type="button">
